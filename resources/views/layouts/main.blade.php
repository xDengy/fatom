    ...
    <header itemscope itemtype="http://schema.org/WPHeader" class="header">
        <div class="navigation">
            <div class="container">
                <div class="menu" itemscope="" itemtype="http://schema.org/SiteNavigationElement">
                    <ul class="">
                        <li class="first @if(url()->current() == route('services')) active @endif">
                            <a href="{{route('services')}}">Услуги</a>
                        </li>
                        <li class="@if(url()->current() == route('about')) active @endif">
                            <a href="{{route('about')}}">О компании</a>
                        </li>
                        <li class="@if(url()->current() == route('delivery')) active @endif">
                            <a href="{{route('delivery')}}">Доставка</a>
                        </li>
                        <li class="last @if(url()->current() == route('contacts')) active @endif">
        ...
        <div itemscope itemtype="http://schema.org/PostalAddress" class="container">
            ...
            <div class="header-contacts">
                ...
                @endphp
                <div>
                    <div id="msMiniCart" class="{{$class}}">
                        <div class="empty">
                            <a href="{{route('cart')}}"><!--  <h5>Корзина</h5>
        ...
        <div class="container">
            <div class="menu submenu" itemscope="" itemtype="http://schema.org/SiteNavigationElement">
                ...
                <ul class="">
                    @foreach($menu as $category)
                        @if($loop->iteration < 4)
                            <li class="@if(url()->current() == route('catalog.category',$category)) active @elseif($loop->first) first @elseif($loop->last) last @endif">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="quick_buy" id="quick_buy">
                    <form class="callback_form ajax_form af_quick_buy quick_buy-form" method="post" action="/one-click-buy" enctype="multipart/form-data">
                        @csrf
                        <input type="hidden" class="qb-input" name="product_id" value="">
                        <input type="hidden" class="qb-input" name="price" value="">
                        <input type="hidden" class="qb-input" name="count" value="">

                        <div class="form-group">
                            <input type="text" class="qb-input form-control" placeholder="Имя *" name="name">
                        </div>
                        <div class="form-group">
                            <input type="tel" class="qb-input form-control" placeholder="Телефон *" id="modal_tel" name="tel">
                        </div>
            ...
            <div class="modal-body mcity">
                ...
                <ul class="list-unstyled cfcity_list">
                    @foreach($domains as $domainKey => $domainValue)
                        @php $serverAddress = request()->server('REQUEST_SCHEME') . '://' . $domainValue->domain . request()->server('REQUEST_URI') @endphp
                        <li><a href="{{ $serverAddress }}" data-city="{{ $domainKey }}">{{ $domainValue->domain_city }}</a></li>
                    @endforeach
                </ul>
            </div>
        </div>
...
@yield('js')
{{--<script type="text/javascript">AjaxForm.initialize({"assetsUrl": "\/assets\/components\/ajaxform\/", "actionUrl": "\/assets\/components\/ajaxform\/action.php", "closeMessage": "\u0437\u0430\u043a\u0440\u044b\u0442\u044c \u0432\u0441\u0435", "formSelector": "form.ajax_form", "pageId": 1});--}}
{{--</script>--}}
